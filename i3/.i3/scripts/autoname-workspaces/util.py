...
def parse_workspace_name(name):
    workspaceNameRE = '(?P<num>\d+):?(?P<shortname>\w+)? ?(?P<icons>.+)?'
    return re.match(workspaceNameRE, name).groupdict()


# Given a dictionary with 'num', 'shortname', 'icons', return the formatted
# name by concatenating them together.
